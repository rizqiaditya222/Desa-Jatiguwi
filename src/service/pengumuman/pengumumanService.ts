   ...
   * @param {AddPengumumanData} pengumumanData - Objek yang berisi judul, konten, dan kategori.
   * @returns {Promise<void>}
   */
  export async function AddPengumuman(pengumumanData: AddPengumumanData): Promise<void> {
    const { title, content, category } = pengumumanData; // Destructure category
   ...
   * @param {Partial<AddPengumumanData & { slug?: string }>} updateData - Objek yang berisi data yang akan diperbarui (judul, konten, kategori, atau slug).
   * @returns {Promise<void>}
   */
