...
import { fetchProfilById, updateProfil } from '@/service/profile/profileService'
...
export default function ManageProfilDesaPage() {
    ...
    (async () => {
      ...
      try {
        ...
        if (!profil) {
          ...
          setPkkUrl((profil as any).pkkUrl ?? '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let nextPkkUrl = pkkUrl;
      let nextDesaUrl = desaUrl;

      if (bpdFile) nextBpdUrl = await uploadImageToSupabase(bpdFile, 'profil/bpd');
      if (pkkFile) nextPkkUrl = await uploadImageToSupabase(pkkFile, 'profil/pkk');
      if (desaFile) nextDesaUrl = await uploadImageToSupabase(desaFile, 'profil/desa');
  ...
  if (loading) return <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">Memuat…</div>;
  ...
  <div className="px-4 sm:px-6 py-8">
    ...
    {/* Form */}
    <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-gray-200 bg-white shadow-sm p-5 sm:p-7">
      {/* Deskripsi */}
      <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> {/* Added grid for horizontal layout */}
        {/* BPD */}
        <div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Gambar BPD</label>
            <input type="file" accept="image/*" onChange={onFileChange(setBpdFile)} className="w-full rounded-xl border border-gray-300 px-3 py-2" />
          ...
          <div className="mt-4 flex items-center justify-center border rounded-xl p-2 bg-gray-50">
            {bpdFile ? (
              <img src={URL.createObjectURL(bpdFile)} alt="Preview BPD" className="max-h-40 rounded-lg object-contain" />
            ) : bpdUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={bpdUrl} alt="BPD" className="max-h-40 rounded-lg object-contain" />
            ) : (
        ...
        <div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Gambar PKK</label>
            <input type="file" accept="image/*" onChange={onFileChange(setPkkFile)} className="w-full rounded-xl border border-gray-300 px-3 py-2" />
            <p className="text-xs text-gray-500 mt-1">Format: JPG/PNG/WebP, maks 5MB.</p>
          </div>
          <div className="mt-4 flex items-center justify-center border rounded-xl p-2 bg-gray-50">
            {pkkFile ? (
              <img src={URL.createObjectURL(pkkFile)} alt="Preview PKK" className="max-h-40 rounded-lg object-contain" />
            ) : pkkUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={pkkUrl} alt="PKK" className="max-h-40 rounded-lg object-contain" />
            ) : (
              <span className="text-sm text-gray-500">Belum ada gambar</span>
            )}
          </div>
        ...
        <div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Gambar Desa</label>
            <input type="file" accept="image/*" onChange={onFileChange(setDesaFile)} className="w-full rounded-xl border border-gray-300 px-3 py-2" />
            <p className="text-xs text-gray-500 mt-1">Format: JPG/PNG/WebP, maks 5MB.</p>
          </div>
          <div className="mt-4 flex items-center justify-center border rounded-xl p-2 bg-gray-50">
            {desaFile ? (
              <img src={URL.createObjectURL(desaFile)} alt="Preview Desa" className="max-h-40 rounded-lg object-contain" />
            ) : desaUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={desaUrl} alt="Desa" className="max-h-40 rounded-lg object-contain" />
            ) : (
              <span className="text-sm text-gray-500">Belum ada gambar</span>
            )}
          </div>
  ...
  <div className="mt-3 border rounded-xl p-3 bg-gray-50">
    {ytUrl && ytId ? (
      ...
      <p className="text-sm text-red-600">URL YouTube tidak valid. Periksa kembali.</p>
    ) : (
      <p className="text-sm text-gray-500">Masukkan URL YouTube untuk melihat preview.</p>
    )}
  </div>
</div>
      ...
      <div className="flex gap-3">
        ...
        <Link href={BASE_PATH} className="flex-1 text-center rounded-xl bg-gray-100 text-gray-800 px-4 py-2.5 font-medium hover:bg-gray-200">
          Batal
        </Link>
      </div>
    </form>
